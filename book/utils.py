...
def get_file_handler(file_name):
    ...
    if not xrootd_src:
        ...
        }  # otherwise the memory maps overload available Vmem
    elif xrootd_src:
...
def get_features_labels(
    file_name, features, spectators, labels, remove_mass_pt_window=True, entry_stop=None
):
    # load file
...
def make_image(feature_array, n_pixels=224, img_ranges=[[-0.8, 0.8], [-0.8, 0.8]]):
    ...
    for i in range(len(wgt)):
        hist2d, xedges, yedges = np.histogram2d(
            x[i], y[i], bins=[n_pixels, n_pixels], range=img_ranges, weights=wgt[i]
        )
        img[i] = hist2d
